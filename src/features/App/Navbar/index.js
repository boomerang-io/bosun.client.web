  ...
  <LeftSideNav isOpen={isOpen}>
    <SideNav expanded={isOpen} isChildOfHeader={true}>
      <SideNavItems>
        ...
        <SideNavLink large onClick={onMenuClose} href={`${BASE_APPS_ENV_URL}/scorecard/`} renderIcon={ReportData16}>
          Scorecard
        </SideNavLink>
        <SideNavLink large onClick={onMenuClose} href={`${BASE_APPS_ENV_URL}/insights/`} renderIcon={ChartScatter16}>
          Insights
        </SideNavLink>
...
function Navbar({ navigationState, userState }) {
  if (navigationState.data && userState.data) {
    return (
      <UIShell
        {...defaultUIShellProps}
